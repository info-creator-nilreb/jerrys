...
export function ProductPdpStickyAtcBar({
  ...
  return createPortal(
    <div
      className="fixed inset-x-0 bottom-0 z-[600000] border-t border-(--surface-muted) bg-white/98 pb-[env(safe-area-inset-bottom)] shadow-[0_-4px_16px_rgb(0_0_0/0.08)] backdrop-blur-sm md:hidden"
      role="region"
    ...
    >
      <div className="mx-auto flex min-h-[3.25rem] max-w-6xl items-center gap-3 px-4 py-2.5">
        ...
        <div className="flex min-w-0 flex-1 flex-col justify-center">
          <p className="truncate text-xs font-semibold leading-tight text-(--foreground-heading)">
            {productTitle}
          </p>
          <p className="text-sm font-semibold leading-tight text-primary">{priceFormatted}</p>
        </div>
        <AddToCartForm
          productId={productId}
