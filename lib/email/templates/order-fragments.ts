...
/** CTA-Button Sendungsverfolgung; leer wenn keine Tracking-URL. */
export function orderTrackingCtaHtml(
  trackUrl: string | null,
  branding: TransactionalEmailBranding,
): string {
  const url = trackUrl?.trim();
  if (!url) return "";
  return transactionalCtaButton(url, "Sendungsverfolgung", branding);
}
