...
| `AUTH_SECRET` | Auth.js (min. 32 Zeichen, z. B. `openssl rand -base64 32`) — **für Preview und Production** getrennt setzen (Häkchen in Vercel). Alias: `NEXTAUTH_SECRET`. Nach Änderung **Redeploy** (Build/Edge müssen das Secret kennen; bei „Sensitive“-Variablen reicht Speichern ohne neuen Deploy oft nicht). |
