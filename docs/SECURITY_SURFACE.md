...
| `POST /api/checkout/paypal/express-shipping-quote` | Öffentlich (Checkout/Warenkorb) | Versandquote für Apple Pay / Express-Sheet vor Pending-Order; **gleiches Rate-Limit** wie create-order |
| `POST /api/checkout/paypal/express-update-shipping` | Öffentlich (Checkout/Warenkorb) | PayPal `onShippingAddressChange`: Pending-Order und Betrag an Lieferland anpassen; **gleiches Rate-Limit** wie create-order |
...
| `POST /api/checkout/paypal/express-cancel` | Öffentlich (Checkout/Warenkorb) | PayPal Express / Smart Buttons `onCancel`: Pending-Express-Bestellung stornieren, Bestand freigeben; **gleiches Rate-Limit** wie create-order |
...
| `GET`/`POST /api/internal/commerce-maintenance` | Bearer `CRON_SECRET` (Vercel Cron), Bearer/`x-commerce-maintenance-secret` (`COMMERCE_MAINTENANCE_SECRET`) | Query/JSON `mode=critical\|full` (Default `full`): `critical` = Stock-Expiry + Workshop-Holds + PayPal-Reconcile + `outboxBacklog`; `full` zusätzlich Outbox-MVP-Publisher, Instagram, Zettle |
...
| Seite `/admin/konto` (geplant, [ADR-0011](./adr/0011-admin-self-service-security.md)) | Admin-Session | Eigenes Passwort ändern, TOTP-MFA enroll/disable |
| Server Actions `app/admin/.../konto/actions.ts` (geplant) | `getAdminSession()` in Action; Rate-Limit | Passwort, MFA-Setup/Confirm/Disable; Outbox `admin_user.*` |
| Seite `/admin/login/mfa` (geplant) | Challenge-Cookie nach gültigem Passwort, noch keine volle Admin-Session | TOTP oder Recovery-Code |
