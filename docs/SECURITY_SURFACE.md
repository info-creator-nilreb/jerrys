...
| `GET`/`POST /api/internal/commerce-maintenance` | Bearer `CRON_SECRET` (Vercel Cron), Bearer/`x-commerce-maintenance-secret` (`COMMERCE_MAINTENANCE_SECRET`) | Query/JSON `mode=critical\|full` (Default `full`): `critical` = Stock-Expiry + Workshop-Holds + PayPal-Reconcile + `outboxBacklog`; `full` zusätzlich Outbox-MVP-Publisher, Instagram, Zettle, Shopify-Produktbild-Nachzug (`shopifyImages`) |
