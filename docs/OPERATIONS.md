...
- **Supabase + Vercel:** Runtime-`DATABASE_URL` = Transaction Pooler (`:6543` + typ. `?pgbouncer=true`). Session Pooler (`:5432`, `pool_size` oft 15) nicht für Serverless-Runtime — sonst `EMAXCONNSESSION`. App-Pool ist auf Vercel auf `max: 1` begrenzt (`lib/db/pg-pool-config.ts`); Idle 5 s + `allowExitOnIdle`; Override `PG_POOL_MAX`. Migrationen über `DIRECT_DATABASE_URL`.
