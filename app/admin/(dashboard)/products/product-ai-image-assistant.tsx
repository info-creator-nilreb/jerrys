...
export function ProductAiImageAssistant({
  ...
  const error = genState?.error || confirmState?.error || altState?.error;
  const message =
    (!error && (confirmState?.message || genState?.message || altState?.message)) || null;
  const draftReady = Boolean(genState?.ok && genState.previewSrc);
  const pending = genPending || confirmPending || altPending;
  const draftAltDefault = genState?.draftAltText?.trim() || `${productTitle.slice(0, 80)} – Produktbild`;
  ...
  return (
    <section className="rounded-xl border border-[#e8eaed] bg-white p-6 shadow-sm">
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-[#6b7280]">
            Neues Produktbild erzeugen, prüfen und erst nach Bestätigung dauerhaft speichern.
            Urheber-, Marken- und Wahrheitsprüfung bleibt deine Verantwortung.
          </p>
        </div>
      </div>
      ...
      {draftReady && genState?.previewSrc ? (
        <div className="mt-6 space-y-4 rounded-lg border border-[#e8eaed] bg-[#f7f8fa] p-4">
          <p className="text-xs font-medium text-[#374151]">
            Vorschau (temporär)
            {genState.model ? (
              <span className="ml-2 font-normal text-[#6b7280]">Modell: {genState.model}</span>
            ) : null}
          </p>
          ...
          <form action={confirmAction} className="space-y-3">
            ...
            <input
              ...
              value={genState.temporaryImageUrl ?? ""}
            />
            <input
              type="hidden"
              name="temporaryImageBase64"
              value={genState.temporaryImageBase64 ?? ""}
            />
            <div>
